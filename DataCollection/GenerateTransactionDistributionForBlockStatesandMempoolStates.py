...
import sys
import joblib
##  Ratio(feerate<3 )=6%---> FeerateDistributionSystem_UnderBlockInterval_-1.pdf
#### When (Feerate>85), 95% tx can Be confirmed in the next block-->FeerareConfrimationRationUnderDifferentBlock.py
sys.path.append("..")
import G_Variables
...
def GenerateMemFeerateVector(start_selection,bins,feerate_startbound,feerate_space):
    ...
    pd_contents.to_csv('../'+IntervalLabel+'MemFeerateVectorTimeBlock'+str(start_selection)+'.csv',index=False,header=False)
...
def GenerateBlockFeerateVector(start_selection, bins,feerate_startbound,feerate_space):
    ...
    pd_contents.to_csv('../'+IntervalLabel+'FeerateVectorTimeBlock'+str(start_selection)+'.csv',index=False,header=False)
