...
def TransformerModel(num_heads,ff_dim,FC_units,  lstmtimestamps, blockFeatureDim, mempoolDim, txFeatureDim):
    ...
    model2 = keras.Model(inputs=[model2_input1, model2_input2, model2_auxiliary_input], outputs=model2_predictions)
...
strain_txOutputList = np_utils.to_categorical(trn_Y,num_classes=clusterNumber) #convert integers to dummy variables (one hot encoding)
...
filepath_fel = result_path + SelectionModule + lossfunction+str(lastUpdateHeight)+str(clusterNumber)+'.h5'
