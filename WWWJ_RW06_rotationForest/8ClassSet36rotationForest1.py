...
def train_test_once_rotationForest(X_train, y_train, X_test, y_test,estimators):
    rof = RotationForestClassifier(n_estimators=estimators)
...
def get_mean_results(estimators, X_train, y_train, X_test, y_test, time):
    ...
    for i in range(time):  # time = repeat time for experiments
        predictions = train_test_once_rotationForest(X_train, y_train, X_test, y_test, estimators)
...
def rotation_forest(txfile,   classes,  estimators, time):
...
if __name__ == '__main__':
    ...
    #pool = mp.Pool(processes=1, maxtasksperchild=1)
        rotation_forest(txfile,  clusterNumber,  estimators, time)
