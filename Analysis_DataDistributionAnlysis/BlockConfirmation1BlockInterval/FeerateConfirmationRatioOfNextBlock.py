...
def GenerateFeerateTimePoints(TestGroup,WaitingBlocks):
    ...
    ratio=0
    ratioList=[]
    while startfeerate<feerate_Max:

        ratio_nextBlock=txdata[(txdata[feerateintx]>=startfeerate)&(txdata[waitingblockintx]<=WaitingBlocks)]
        next_sample=ratio_nextBlock.shape[0]
        ratio_moreBlock = txdata[txdata[feerateintx] >= startfeerate]
        more_sample=ratio_moreBlock.shape[0]
        ratio=next_sample/more_sample
        ratioList.append(ratio)
        print('feerate:'+str(startfeerate)+' ratio'+str(ratio))
        startfeerate = startfeerate + space
    return ratioList
...
points=int(feerate_Max)
xData=range(1, points)


for WaitingBlocks in [1,2,3]:
    ratioList=GenerateFeerateTimePoints(TestGroup,WaitingBlocks)
    plt.plot(xData, ratioList,label='waitingblock<='+str(WaitingBlocks))
    for samplepoint in [2,5,10,20,40,80,100]:
        x0 = xData[samplepoint]
        y0 = ratioList[samplepoint]
        plt.scatter(x0, y0, s=20)
        plt.text(x0, y0, (x0, round(y0, 2)))




plt.ylabel('Confirmation ratio')
plt.xlabel('Transaction feerate')
plt.legend()
plt.show()
plt.savefig('FeerateConfirmationRatioOfNextBlock' + '.pdf')
...
GenerateFeerateTimePoints('16',10000000)
