...
def GenerateFeerateTimePoints(TestGroup,MaxTimeInterval):
    ...
    while startfeerate<feerate_Max:
        ...
        ratio_nextBlock=txdata[(txdata[feerateintx]>=startfeerate)&(txdata[waitingblockintx]==1)]
    ...
    plt.plot(xData, ratioList[0:points])
    ...
    plt.text(x0, y0, (x0, round(y0,2)), color='r')
    plt.scatter(x1, y1, s=50)
    plt.text(x1, y1, (x1, round(y1,2)), color='r')

    # plt.annotate('7590', xy=(x0,y0), xycoords='data', xytext=(+30, -30),
    #              textcoords='offset points', fontsize=6,
    #              arrowprops=dict(arrowstyle='->', connectionstyle="arc3,rad=.2"))

    plt.ylabel('Confirmation ratio of 1-block interval')
    plt.xlabel('Transaction feerate')
    plt.savefig('ConfirmationDistribution' + '.pdf')
