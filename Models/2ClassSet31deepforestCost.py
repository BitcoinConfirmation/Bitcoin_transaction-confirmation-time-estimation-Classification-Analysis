...
def train_test_df(txfile, smallest_classes, classes, discretization, cost_sensitiveLabel, estimators, repeattime):
    ...
    train_txfeatureList1,train_txOutputList1,strain_blockSeqList1,strain_memSeqList1,Df_label=txDatasetConstruction(txfile,blockfile,memfile,lstmtimestamps,CurrentHeight-training_blocks,CurrentHeight+total_EstimateBlock,classes, discretization, smallest_classes)
    #######Scale Features
    ...
    train_txfeatureList,train_txOutputList,strain_blockSeqList,strain_memSeqList,Df_label=txDatasetConstruction(txfile,blockfile,memfile,lstmtimestamps,CurrentHeight-training_blocks,CurrentHeight,classes, discretization, smallest_classes)
    #######Scale Features
    ...
    test_txfeatureList, test_txOutputList,stest_blockSeqList,stest_memSeqList,Df_label = txDatasetConstruction(txfile,blockfile,memfile,lstmtimestamps,CurrentHeight, CurrentHeight + total_EstimateBlock,classes, discretization, smallest_classes)
    ...
    if cost_sensitiveLabel:
        ...
        flag='noncost'
    df_save.to_csv(os.path.abspath('..').replace('\\','/') + '/NeuarlResult/DeepForestResults/Set'+str(TestGroup)+'deepForest'+str(CurrentHeight)+str(classes)+flag+str(estimators)+'.csv',
...
if __name__ == '__main__':
    ...
    #pool = mp.Pool(processes=1, maxtasksperchild=1)
        train_test_df(txfile,smallest_classes, classes, discretization, cost_sensitiveLable, estimators, repeattime)
