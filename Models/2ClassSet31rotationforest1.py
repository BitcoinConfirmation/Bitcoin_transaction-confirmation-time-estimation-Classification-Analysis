...
def train_test_once(X_train, y_train, X_test, y_test):
...
def get_mean_results(X_train, y_train, X_test, y_test, time):
    ...
    for i in range(time):  # time = repeat time for experiments
        predictions = train_test_once(X_train, y_train, X_test, y_test)
...
def rotation_forest(txfile,  classes,  time):
    ...
    acc,prec_macro,prec_micro,prec_wht,recall_macro,recall_micro,recall_wht,f1_macro,f1_micro,f1_wht = get_mean_results( X_train, y_train, X_test, y_test, time)
    df_save = pd.DataFrame([[classes,  time,
                             acc,prec_macro,prec_micro,prec_wht,recall_macro,recall_micro,recall_wht,f1_macro,f1_micro,f1_wht]])

    df_save.to_csv(os.path.abspath('..').replace('\\','/')  + '/NeuarlResult/Forestresults/Set'+str(TestGroup)+'rotationForest'+str(CurrentBlock)+str(classes)+'.csv',
...
if __name__ == '__main__':
    ...
    rotation_forest(txfile, classes, time)
