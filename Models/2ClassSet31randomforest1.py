...
def random_forest(txfile,   classes,  estimators, time):
    ...
    df_save.to_csv(os.path.abspath('..').replace('\\','/') + '/NeuarlResult/Forestresults/Set'+str(TestGroup)+'randomForest'+str(CurrentBlock)+str(classes)+'.csv',
...
if __name__ == '__main__':
    ...
    #pool = mp.Pool(processes=1, maxtasksperchild=1)
        random_forest(txfile,  classes,  estimators, time)
