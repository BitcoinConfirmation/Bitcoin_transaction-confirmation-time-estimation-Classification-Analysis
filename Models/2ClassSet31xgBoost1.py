...
def xgBoostTime(txfile, classes, plst, time):
    ...
    df_save.to_csv(os.path.abspath('..').replace('\\','/') + '/NeuarlResult/xgBoostResults/Set'+str(TestGroup)+'xgBoost'+str(CurrentBlock)+str(classes)+'.csv',
...
if __name__ == '__main__':
    ...
    #pool = mp.Pool(processes=1, maxtasksperchild=1)
        xgBoostTime(txfile,  classes, plst, time)
